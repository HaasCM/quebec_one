...
if (!(getmarkercolor _mkr == StopColor)) then
{
	...
	//SPAWN CHOPPER
	private _heloDirection = random 360;
	private _heloDistance = 200 + random 200;
	private _heloLandingArea = (markerpos _mkr) vectorAdd [_heloDistance * sin _heloDirection, _heloDistance * cos _heloDirection, 0];
	for "_counter" from 1 to _fGrps do
	{
		...
		if ((_fSize select 0) > 0) then
		{
			...
			[_cargoGrp] call SERVER_RegisterDeaths;
			_fGroup set [count _fGroup,_cargoGrp];
			private _heloPosition = _heloLandingArea vectorAdd [-150 + random 300, -150 + random 300, 0];
			null = [_heloPosition,_fGroup,_counter,_mkr] execvm "scripts\eos\functions\TransportUnload_fnc.sqf";
		}else{
	...
	while {_eosAct} do
	{
		...
		if (!triggeractivated _eosActivated || getmarkercolor _mkr == StopColor) exitwith
			{
				if (_debug) then {if (!(getmarkercolor _mkr == StopColor)) then {hint "Restarting Zone AND deleting units";}else{hint "EOS zone deactivated";};};
...
//							{deleteVehicle _x} forEach (_crew);
				...
				private _markerPosition = getMarkerPos _mkr;
				private _markerRadius = ((getMarkerSize _mkr) select 0) * 1.2;
				private _radiusSquared = _markerRadius * _markerRadius;
				{
					if (_x distanceSqr _markerPosition < _radiusSquared) then
					{
						deleteVehicle _x;
					};
				} forEach allDead;
		...
		if (triggeractivated _clear and triggeractivated _taken and !_civZone) exitwith
			{
				...
				while {triggeractivated _eosActivated AND !(getmarkercolor _mkr == StopColor)} do
				{
