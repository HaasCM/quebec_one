...
#define NUMBER_VISIBLE_SEATS 15
...
CREW_UpdateCrewDisplay =
{
	...
	else
	{
		...
		{
			private _name = if (isNull (_x select 0)) then { "" } else { name (_x select 0) };
			private _color = if (_x select 0 == player) then { _playerColor } else { _otherColor };
