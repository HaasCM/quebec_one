...
{
	{
		...
		if (_unitIsInGreenZone) then
		{
			if (not (_x in GZ_GreenZoneUAVs)) then
			{
				...
				titleText [_enterMessage, "PLAIN", 0.3];
			};
		}
		else
		{
			if (_x in GZ_GreenZoneUAVs) then
			{
				...
				titleText [_leaveMessage, "PLAIN", 0.3];
			};
		};
	} forEach allUnitsUAV;
};
...
else
{
	...
	if (_isUAVOperator) then
	{
		_enterGreenZone =
		{
			...
			// Non-curators may not fire in green zones
			if (CLIENT_CuratorType == "") then
			{
				private _handlerIndex = _unit addEventHandler ["FiredMan",
					{
						...
						if (not unitIsUAV _unit) then
						{
							...
							titleText [GREEN_ZONE_FIRING_MESSAGE, "BLACK IN", 3];
						}
						else
						{
			...
			if (alive _unit && animationstate player != "AinjPpneMstpSnonWrflDnon") then // alive and not 'unconscious'
			{
				titleText [ENTER_GREEN_ZONE_MESSAGE, "PLAIN", 0.3];
			};
		};
		...
		_leaveGreenZone =
		{
			...
			if (alive _unit && animationstate player != "AinjPpneMstpSnonWrflDnon") then
			{
				titleText [LEAVE_GREEN_ZONE_MESSAGE, "PLAIN", 0.3];
			};
		};
	}
