...
// EAST CSAT FACTION
	if (_faction==0) then {
	_InfPool=	["O_Soldier_AR_F","O_Soldier_LAT_F","O_Soldier_F","O_Soldier_lite_F","O_soldier_M_F","O_medic_F","O_soldier_PG_F"];		
	_ArmPool=	["O_APC_Wheeled_02_rcws_F","O_APC_Tracked_02_cannon_F", "O_MRAP_02_hmg_F"];
	_MotPool=	["O_Truck_03_covered_F", "O_Truck_02_covered_F", "O_Truck_03_transport_F", "O_Truck_02_transport_F"];
	_ACHPool=	["O_Heli_Light_02_F"]; // AAF helos don't spawn in with a crew if there are no other AAF forces ["I_Heli_light_03_F"];
...
// WEST NATO FACTION	
	if (_faction==1) then {
	_InfPool=	["B_sniper_F","B_Soldier_A_F","B_Soldier_AAR_F","B_Soldier_AAT_F","B_Soldier_AR_F","B_Soldier_LAT_F","B_soldier_exp_F","B_Soldier_F","B_engineer_F","B_medic_F","B_recon_exp_F","B_recon_F","B_recon_JTAC_F","B_recon_LAT_F","B_recon_M_F","B_recon_medic_F","B_recon_TL_F"];	
	...
	_MotPool=	["B_Truck_01_covered_F","B_Truck_01_transport_F","B_MRAP_01_F","B_MRAP_01_hmg_F","B_Truck_01_medical_F"];
...
// INDEPENDENT AAF FACTION	
	if (_faction==2) then {
	_InfPool=	["I_engineer_F","I_Soldier_A_F","I_Soldier_AAR_F","I_Soldier_AAT_F","I_Soldier_AR_F","I_Soldier_LAT_F","I_Soldier_exp_F","I_soldier_F","I_Soldier_GL_F","I_Soldier_repair_F"];	
	_ArmPool=	["I_APC_Wheeled_03_cannon_F"];
	_MotPool=	["I_MRAP_03_F","I_MRAP_03_hmg_F","I_Truck_02_medical_F"];
	_ACHPool=	[];
...
// EAST CSAT URBAN
	if (_faction==5) then {
	_InfPool=	["O_soldierU_AR_F","O_soldierU_F","O_soldierU_LAT_F","O_SoldierU_GL_F","O_soldierU_M_F","O_soldierU_exp_F","O_soldierU_medic_F","O_recon_F"];
	_ArmPool=	[];
	_MotPool=	[];
	_ACHPool=	[];
	_CHPool=	[];
...
////////////////////////////////////////////////////////////////////////////////////////
if (_type==0 && (count _InfPool > 0)) then {
...
// CREATE ARMOUR & CREW			
if (_type==2 && (count _ArmPool > 0)) then {
...
if (_type==3 && (count _ACHPool > 0)) then {
...
if (_type==4 && (count _CHPool > 0)) then {
...
// CREATE STATIC & CREW						
if (_type==5 && (count _stPool > 0)) then {
...
if (_type==7 && (count _MotPool > 0)) then {
...
if (_type==8 && (count _shipPool > 0)) then
{
	_tempUnit=_shipPool select (floor(random(count _shipPool)));
	_temparray pushBack _tempUnit;
	_tempArray pushBack objNull;
};
				
// CREATE CARGO
if (_type==9 && (count _InfPool > 0)) then {
...
// CREATE DIVER CARGO
if (_type==10 && (count _diverPool > 0)) then
{
	for "_i" from 0 to 4 do
	{
