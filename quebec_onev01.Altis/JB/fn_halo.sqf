_this spawn
{
	...
	// Make sure that the _unit is out of the aircraft.  ARMA doesn't appear to permit teleports
	// directly from all seats.  (e.g. Huron passenger seats)
	moveOut _unit;
	...
	if (_open) then
	{
		waitUntil { not alive _unit || animationState _unit == "halofreefall_non"};
