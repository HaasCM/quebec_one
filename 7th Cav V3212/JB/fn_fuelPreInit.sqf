diag_log "JB_fnc_fuelPreInit start";
...
JBF_R_RetractFuelLine =
{
	...
	{
		private _fuelSupply = _this select 0;
		private _fuelLine = _this select 1;
...
JBF_GetClosestFuelLine =
{
	...
	private _mustBeOpen = _this select 2;
...
JBF_GetFuelLine =
{
	...
	if (_fuelRemaining == 0) exitWith { titleText ["Fuel supply is empty", "BLACK IN", 3] };

	private _closestFuelLine = [_fuelSupply, getPos player, true] call JBF_GetClosestFuelLine;

	private _fuelLineIndex = _closestFuelLine select 0;

	if (_fuelLineIndex == -1) exitWith { titleText ["All available fuel lines are in use", "BLACK IN", 3] };
...
JBF_R_GetFuelLine2 =
{
	...
	{
		private _fuelSupply = _this select 0;
		private _fuelLineIndex = _this select 1;
		private _fuelLinePosition = _this select 2;
...
JBF_R_AttachFuelLine =
{
	...
	else
	{
		private _fuelSupply = _this select 0;
		private _fuelLine = _this select 1;
		private _fuelLineIndex = _this select 2;
		private _fuelConsumer = _this select 3;
		private _fuelHandle = _this select 4;
		private _direction = _this select 5;
...
{
	private _holder = _this select 0;
...
JBF_IsFueling =
{
	private _fuelHandle = _this select 0;
...
JBF_R_TransferFuel =
{
	...
	private _fuelSupply = _this select 1;
	private _fuelPulse = _this select 2;
...
JBF_R_FuelVehicle2 =
{
	...
	else
	{
		private _fuelSupply = _this select 0;
		private _fuelLine = _this select 1;
		private _fuelHandle = _this select 2;
		private _holder = _this select 3;
		private _vehicle = _this select 4;
		...
		[_holder, _vehicle, _fuelHandle, _fuelSupply, _fuelLine] spawn
		{
			...
			private _vehicle = _this select 1;
			private _fuelHandle = _this select 2;
			private _fuelSupply = _this select 3;
			private _fuelLine = _this select 4;
...
JBF_SetupClient =
{
	...
	_unit addAction ["Get fuel line", { [_this select 0] call JBF_GetFuelLine }, nil, 10, false, true, "", "[cursorTarget] call JBF_GetFuelLineCondition"];
};
...
diag_log "JB_fnc_fuelPreInit end";