...
if (isNil { OO_##class }) then { diag_log "OO_OVERRIDE_METHOD: undefined class" };\
if (isNil { OO_##parent }) then { diag_log "OO_OVERRIDE_METHOD: undefined parent" };\
if (isNil { OO_##parent##_Method_##method }) then { diag_log "OO_OVERRIDE_METHOD: undefined parent method" };\
if (isNil { code }) then { diag_log "OO_OVERRIDE_METHOD: undefined code" }
#else
#define OO_OVERRIDE_METHOD_CHECK(class,parent,method,code)
#endif
...
if (isNil{ OO_##class }) then{ diag_log "OO_DEFINE_METHOD: undefined class" }; \
if (isNil { code }) then { diag_log "OO_DEFINE_METHOD: undefined code" }
#else
#define OO_DEFINE_METHOD_CHECK(class,method,code)
#endif
...
if (isNil { OO_##class }) then { diag_log "OO_DEFINE_PROPERTY: undefined class" };\
if (isNil { type }) then { diag_log "OO_DEFINE_PROPERTY: undefined type" };\
if (isNil { default }) then { diag_log "OO_DEFINE_PROPERTY: undefined default" }
#else
#define OO_DEFINE_PROPERTY_CHECK(class,property,type,default)
#endif
...
OO_CLASSNAME_PROPERTY_TABLE(class) pushBack [(type),(default)]
...
OO_ValueDescription =
{
	...
	{
		case typeName[]: { format ["ARRAY [%1]", count _value] };
		case typeName{}: { "CODE" };
...
diag_log format ["%1OO_SET: set %2 instance %3 property %4 to %5", if (isNil "_TRACE_INDENT") then { "" } else { _TRACE_INDENT }, #class, if (OO_ISCLASS(class)) then { OO_INSTANCE_ID(instance) } else { "STRUCT" }, #property, [value] call OO_ValueDescription];
#else
#define OO_SET_TRACE(instance,class,property,value)
#endif
...
#define OO_REFERENCE(instance) [OO_INSTANCE_ID(instance), OO_INSTANCE_CLASSINDEX(instance)]
#define OO_INSTANCE(reference) ([OO_CLASSINDEX_REFERENCE_TABLE((reference) select 1), (reference) select 0] call REF_GetValue)


#ifdef OO_TRACE
#define OO_CREATECLASS_TRACE(class)\
diag_log format ["CONSTRUCTOR: %1 %2", OO_CLASSINDEX_CLASS_NAME(OO_GET(_instance,Root,Class)), OO_INSTANCE_ID(_instance)]
#else
#define OO_CREATECLASS_TRACE(class)
#endif
...
diag_log format ["CONSTRUCTOR: %1", OO_CLASSINDEX_CLASS_NAME(OO_GET(_instance,RootStruct,Class))]
#else
#define OO_CREATESTRUCT_TRACE(class)
#endif
...
{\
	...
	private _result = (_this call method##_Actual); \
	private _elapsed = diag_tickTime - _start;\
	diag_log format["%1%2 %3 %4", _TRACE_INDENT, #method, if (isNil "_result") then { "nil" } else { [_result] call OO_ValueDescription }, _elapsed];\
